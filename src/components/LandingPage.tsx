...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  // GitHub Real Stars count for rudresh05/syntra
  const [repoStars, setRepoStars] = useState<number | null>(null);

  useEffect(() => {
    fetch('https://api.github.com/repos/rudresh05/syntra')
      .then((res) => {
    ...
    <div className="min-h-screen bg-[#030712] text-slate-100 flex flex-col selection:bg-emerald-500 selection:text-slate-950 relative overflow-x-hidden">
      ...
      <header className="border-b border-slate-800/80 bg-[#030712]/85 backdrop-blur-2xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            <a
              href="https://github.com/rudresh05/syntra"
              target="_blank"
              rel="noreferrer"
              className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold bg-slate-900/90 border border-slate-800 text-slate-300 hover:text-white hover:border-slate-700 transition-all shadow-sm group"
              title="Star rudresh05/syntra on GitHub"
            >
              <Github className="w-3.5 h-3.5 text-slate-400 group-hover:text-white transition-colors" />
              <span className="hidden sm:inline text-slate-300">Star</span>
              <div className="flex items-center space-x-1 pl-1 sm:pl-1.5 sm:border-l sm:border-slate-800">
                <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                <span className="font-mono text-white text-[11px] font-bold">
                  {repoStars !== null ? repoStars : 0}
                </span>
              </div>
            </a>
      ...
      <section className="relative pt-12 pb-10 md:pt-20 md:pb-20 overflow-hidden z-10">
        <div className="max-w-5xl mx-auto px-4 text-center space-y-5 sm:space-y-7">
          {/* Floating GitHub Repo Badge with Real Stars */}
          <a
            href="https://github.com/rudresh05/syntra"
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center space-x-2 px-3.5 sm:px-4 py-1.5 rounded-full bg-slate-900/90 border border-emerald-500/40 text-emerald-300 text-[11px] sm:text-xs font-medium shadow-xl backdrop-blur-md hover:border-emerald-400 hover:scale-[1.02] active:scale-[0.98] transition-all group cursor-pointer animate-float"
            title="View rudresh05/syntra on GitHub"
          >
            <Github className="w-3.5 h-3.5 text-emerald-400" />
            <span className="text-slate-300 font-semibold font-mono">rudresh05/syntra</span>
            <span className="text-slate-600">•</span>
            <span className="flex items-center space-x-1 text-amber-400 font-mono font-bold">
              <Star className="w-3 h-3 fill-amber-400" />
              <span>{repoStars !== null ? repoStars : 0}</span>
            </span>
            <span className="text-slate-600">•</span>
            <span className="text-emerald-400 font-bold group-hover:underline flex items-center gap-1">
              Star on GitHub <ExternalLink className="w-3 h-3 inline" />
            </span>
          </a>
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center space-x-1.5 text-slate-400 hover:text-white transition-colors group"
            >
              <Github className="w-4 h-4 text-slate-400 group-hover:text-white" />
              <span className="font-mono">rudresh05/syntra</span>
              <span className="flex items-center text-amber-400 text-[11px] font-mono font-bold bg-slate-900 px-2 py-0.5 rounded border border-slate-800 group-hover:border-slate-700">
